...
public class SolutionSet implements Serializable {
  ...
  */
      ...
      if (size()>0) {
        ...
        for (Solution aSolutionsList_ : solutionsList_) {
          for (int j = 0; j < numberOfVariables; j++) {
            try {
				bw.write(aSolutionsList_.getDecisionVariables()[j].getLowerBound() + " " +aSolutionsList_.getDecisionVariables()[j].getUpperBound());
			} catch (JMException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
          	
			/*binaryVariable = (BinaryReal)aSolutionsList_.getDecisionVariables()[j];
  ...
  public void printVariablesNamesToFile(String path){
	    try {
	      FileOutputStream fos   = new FileOutputStream(path)     ;
	      OutputStreamWriter osw = new OutputStreamWriter(fos)    ;
	      BufferedWriter bw      = new BufferedWriter(osw)        ;            
